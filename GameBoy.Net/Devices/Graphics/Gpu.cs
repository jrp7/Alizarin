...
namespace GameBoy.Net.Devices.Graphics
{
    ...
    public class Gpu : IGpu
    {
        ...
        private RenderChecksum _renderChecksum; // the checksum of the last render.
        ...
        private async Task PaintLoop()
        {
            while (!_disposed)
            {
                var result = await _paintingTaskCompletionSource.Task;
                if (!result)
                {
                    break;
                }
        ...
        private void Paint()
        {
            ...
            var renderState = new RenderContext(renderSettings, tileSet, backgroundTileMap, windowTileMap, spriteOam, spriteTileSet);
            var renderChecksum = renderState.GetChecksum();
            var renderStateChange = renderChecksum.GetRenderStateChange(_renderChecksum);
            ...
            _renderChecksum = renderChecksum;
